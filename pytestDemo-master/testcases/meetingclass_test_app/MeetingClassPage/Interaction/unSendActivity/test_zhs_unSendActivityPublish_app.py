...
class Test_unSendActivityPublish_app():
    """app发布待发布列表中的活动"""

    @allure.story("互动-待发布")
    @allure.description("app发布待发布列表中的活动")
    @allure.title("app发布待发布列表中的活动")
    @pytest.mark.single
    def test_zhs_unSendActivityPublish_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            if result_unSendActivityList_app.response.status_code == 200:
                ...
                if result_unSendActivityList_app.response.json()["rt"]["votesList"] != []:
                    votesList = random.sample(result_unSendActivityList_app.response.json()["rt"]["votesList"], 1)[0]
                    dataId = votesList["dataId"]
                    dataType = votesList["dataType"]
                    logger.info("unSendActivityPublish_app")
                    result_unSendActivityPublish_app = unSendActivityPublish_app(dataId, dataType, meetCourseId, uuid,
                                                                                 access_token=access_token)
                    assert result_unSendActivityPublish_app.response.status_code == 200
                    if result_unSendActivityPublish_app.response.status_code == 200:
                        logger.info(result_unSendActivityPublish_app.response.json()["rt"])
                else:
                    logger.info("没有待发布活动")
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_unSendActivityPublish_app.py"])
