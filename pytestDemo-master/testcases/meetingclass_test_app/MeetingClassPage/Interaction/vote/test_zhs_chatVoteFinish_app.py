...
class Test_chatVoteFinish_app():
    """app结束投票"""

    @allure.story("互动-投票")
    @allure.description("app结束投票")
    @allure.title("app结束投票")
    @pytest.mark.single
    def test_zhs_chatVoteFinish_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            if voteids == []:
                ...
                logger.info("chatVoteFinish_app")
                result_chatVoteFinish_app = chatVoteFinish_app(uuid, voteId, access_token=access_token)
                assert result_chatVoteFinish_app.response.status_code == 200
                if result_chatVoteFinish_app.response.status_code == 200:
                    logger.info(result_chatVoteFinish_app.response.json()["rt"])
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatVoteFinish_app.py"])
