...
class Test_chatVoteDetailOptionCount_app():
    """app获取各选项投票人数"""

    @allure.story("互动-投票")
    @allure.description("app获取各选项投票人数")
    @allure.title("app获取各选项投票人数")
    @pytest.mark.single
    def test_zhs_chatVoteDetailOptionCount_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            if voteids == []:
                ...
                if result_chatVoteDetailOptionCount_app.response.status_code == 200:
                    logger.info("投票id为{}，选项详情及人数为：{}".format(voteId,
                                                             result_chatVoteDetailOptionCount_app.response.json()["rt"][
                                                                 "voteOptions"]))
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatVoteDetailOptionCount_app.py"])
