...
class Test_chatVoteDetail_app():
    """app获取投票详情"""

    @allure.story("互动-投票")
    @allure.description("app获取投票详情")
    @allure.title("app获取投票详情")
    @pytest.mark.single
    def test_zhs_chatVoteDetail_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatVoteDetail_app.py"])
