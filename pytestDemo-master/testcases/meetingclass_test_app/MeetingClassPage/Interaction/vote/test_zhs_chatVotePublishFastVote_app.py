...
class Test_chatVotePublishFastVote_app():
    """app开始快速投票"""

    @allure.story("互动-投票")
    @allure.description("app开始快速投票")
    @allure.title("app开始快速投票")
    @pytest.mark.single
    def test_zhs_chatVotePublishFastVote_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            type = randomRangeNum(1, 3)  # 1 选择 2 判断 3 多选
            if type == 2:
                optionCount = 2
            else:
                optionCount = randomRangeNum(2, 9)
            logger.info("chatVotePublishFastVote_app")
            result_chatVotePublishFastVote_app = chatVotePublishFastVote_app(groupIds, limitTime, optionCount, type,
                                                                             uuid, access_token=access_token)
            assert result_chatVotePublishFastVote_app.response.status_code == 200
            if result_chatVotePublishFastVote_app.response.status_code == 200:
                logger.info(
                    "{}，创建的voteid为{}".format(result_chatVotePublishFastVote_app.response.json()["rt"]["resultMessage"],
                                             result_chatVotePublishFastVote_app.response.json()["rt"]["voteId"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatVotePublishFastVote_app.py"])
