...
class Test_findCoursePrepareChildenApp():
    """app查询当前备课计划的所有子卡片"""

    @allure.story("互动-备课")
    @allure.description("app查询当前备课计划的所有子卡片")
    @allure.title("app查询当前备课计划的所有子卡片")
    @pytest.mark.single
    def test_zhs_findCoursePrepareChildenApp(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_findCoursePrepareChildenApp.py"])
