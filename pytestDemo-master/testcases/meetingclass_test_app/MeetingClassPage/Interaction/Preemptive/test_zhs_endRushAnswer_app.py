...
class Test_endRushAnswer_app():
    """app结束抢答"""

    @allure.story("互动-抢答")
    @allure.description("app结束抢答")
    @allure.title("app结束抢答")
    @pytest.mark.single
    def test_zhs_endRushAnswer_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_endRushAnswer_app.py"])
