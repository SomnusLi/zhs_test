...
class Test_answerOrTroubleMemberList_app():
    """app问题想回答/有疑问学生列表"""

    @allure.story("互动-答疑")
    @allure.description("app问题想回答/有疑问学生列表")
    @allure.title("app问题想回答/有疑问学生列表")
    @pytest.mark.single
    def test_zhs_answerOrTroubleMemberList_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            if result_checkExistQuestion_app.response.json()["rt"]["result"] == 1:
                ...
                if result_checkQuestionAndJoinNum_app.response.json()["rt"]["questionNum"] > 0:
                    ...
                    logger.info("answerOrTroubleMemberList_app")
                    questionId = random.sample(resilt_questionList_app.response.json()["rt"]["questionList"], 1)[0][
                        "questionId"]
                    pageNum = 1
                    pageSize = 1000
                    type = 1
                    result_answerOrTroubleMemberList_app = answerOrTroubleMemberList_app(groupId, pageNum, pageSize,
                                                                                         questionId, type,
                                                                                         access_token=access_token)
                    assert result_answerOrTroubleMemberList_app.response.status_code == 200
                    logger.info(
                        "参与的学生信息:{}".format(result_answerOrTroubleMemberList_app.response.json()["rt"]["memberList"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_answerOrTroubleMemberList_app.py"])
