...
class Test_questionDetail_app():
    """app提问下的问题详情"""

    @allure.story("互动-答疑")
    @allure.description("app提问下的问题详情")
    @allure.title("app提问下的问题详情")
    @pytest.mark.single
    def test_zhs_questionDetail_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            if result_checkExistQuestion_app.response.json()["rt"]["result"] == 1:
                ...
                if result_checkQuestionAndJoinNum_app.response.json()["rt"]["questionNum"] > 0:
                    ...
                    logger.info(
                        "问题内容为:{}".format(result_questionDetail_app.response.json()["rt"]["questionDetail"]["content"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_questionDetail_app.py"])
