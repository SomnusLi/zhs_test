...
class Test_openQuestionDetail_app():
    """app查询答疑详情"""

    @allure.story("用例--app查询答疑详情")
    @allure.description("该用例是app查询答疑详情")
    @allure.title("测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_openQuestionDetail_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            if result_checkExistQuestion_app.response.json()["rt"]["result"] == 1:
                ...
                logger.info("openQuestionDetail_app")
                result_openQuestionDetail_app = openQuestionDetail_app(rushQuestionId, access_token=access_token)
                assert result_openQuestionDetail_app.response.status_code == 200
                if result_openQuestionDetail_app.response.status_code == 200:
                    logger.info("{}".format(result_openQuestionDetail_app.response.json()["rt"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_openQuestionDetail_app.py"])
