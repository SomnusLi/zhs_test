...
class Test_findMeetCourseLiveStatus_app():
    """app查询正在进行中的见面课信息"""

    @allure.story("用例--app查询正在进行中的见面课信息")
    @allure.description("该用例是app查询正在进行中的见面课信息")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_findMeetCourseLiveStatus_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            logger.info("findMeetCourseLiveStatus_app")
            fromType = 1
            role = 1
            result_findMeetCourseLiveStatus_app = findMeetCourseLiveStatus(meetCourseId, role, fromType, uuid,
                                                                           access_token=access_token)
            assert result_findMeetCourseLiveStatus_app.response.status_code == 200
            logger.info(result_findMeetCourseLiveStatus_app.response.json()["rt"])
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_findMeetCourseLiveStatus_app.py"])
