...
class Test_getMeetCourseFileList_app():
    """app查询投屏课件历史"""

    @allure.story("见面课学习资源")
    @allure.description("app查询投屏课件历史")
    @allure.title("app查询投屏课件历史")
    @pytest.mark.single
    def test_zhs_getMeetCourseFileList_app(self, login_fixture_teacher_app):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_getStartingMeetCourseList_app.response.json()["rt"] != []:
            ...
            logger.info("本次见面课已选择过的课件为：{}".format(result_getMeetCourseFileList_app.response.json()["rt"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_getMeetCourseFileList_app.py"])
