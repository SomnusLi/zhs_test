...
class Test_findOnGoingSignIdsAndRushQuestionId():
    """查询见面课下进行的签到和提问"""

    @allure.story("用例--查询见面课下进行的签到和提问")
    @allure.description("该用例是查询见面课下进行的签到和提问")
    ...
    def test_zhs_findOnGoingSignIdsAndRushQuestionId(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_findOnGoingSignIdsAndRushQuestionId.py"])
