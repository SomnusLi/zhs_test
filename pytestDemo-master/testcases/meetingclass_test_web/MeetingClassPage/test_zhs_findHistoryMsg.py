...
class Test_findHistoryMsg():
    """根据群组id获取群聊id"""

    @allure.story("见面课信息")
    @allure.description("根据群组id获取群聊id")
    @allure.issue("https://www.zhihuishu.com/", name="点击，跳转到对应BUG的链接地址")
    @allure.testcase("https://www.zhihuishu.com/", name="点击，跳转到对应用例的链接地址")
    @allure.title("根据群组id获取群聊id")
    @pytest.mark.single
    def test_zhs_findHistoryMsg(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            logger.info("getChatroomIdByGroupId")
            ...
            chatroomId = result_getChatroomIdByGroupId.response.json()["rt"]
            logger.info("findHistoryMsg")
            pageSize = 1000
            result_findHistoryMsg = findHistoryMsg(chatroomId, pageSize, uuid, cookies=cookies)
            assert result_findHistoryMsg.response.status_code == 200
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_findHistoryMsg.py"])
