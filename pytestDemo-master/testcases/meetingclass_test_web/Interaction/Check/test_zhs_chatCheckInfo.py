...
class Test_chatCheckInfo():
    """查询签到信息详情"""

    @allure.story("用例--查询签到信息详情")
    @allure.description("该用例查询签到信息详情")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_chatCheckInfo(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
            ...
            if result_screenCourseClassInteractionListV2.response.json()["rt"]["interactionListGoing"] == []:
                ...
                for ListGoing in result_screenCourseClassInteractionListV2.response.json()["rt"][
                    ...
                    if ListGoing["type"] == 1:
                        ...
                        result_chatCheckInfo = chatCheckInfo(checkId, uuid, cookies=cookies)
                        assert result_chatCheckInfo.response.status_code == 200
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatCheckInfo.py"])
