...
class Test_chatCheckStatus():
    """更改学生签到状态"""

    @allure.story("用例--更改学生签到状态")
    @allure.description("该用例更改学生签到状态")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_chatCheckStatus(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatCheckStatus.py"])
