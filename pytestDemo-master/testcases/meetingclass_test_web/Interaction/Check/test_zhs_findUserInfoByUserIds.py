...
class Test_findUserInfoByUserIds():
    """通过学生uuids查询签到/未签的学生相关信息"""

    @allure.story("用例--通过学生uuids查询签到/未签的学生相关信息")
    @allure.description("该用例通过学生uuids查询签到/未签的学生相关信息")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_findUserInfoByUserIds(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
                    ...
                    if ListGoing["type"] == 1:
                        ...
                        for checkType in range(1, 3):
                            ...
                            if result_signUserIdsHistory.response.json()["rt"]["resultMessage"] == "成功":
                                uuids = ""
                                for checkStudentsList in result_signUserIdsHistory.response.json()["rt"][
                                    "checkStudentsList"]:
                                    uuids = uuids + "&{}".format(checkStudentsList)
                                uuids = uuids[1:]
                                result_findUserInfoByUserIds = findUserInfoByUserIds(uuids, groupId, uuid,
                                                                                     cookies=cookies)
                                assert result_findUserInfoByUserIds.response.statuse_code == 200
                            else:
                                logger.info("没有学生")
                    ...
                    if DetailDtos["type"] == 1:
                        ...
                        for checkType in range(1, 3):
                            ...
                            if result_signUserIdsHistory.response.json()["rt"]["resultMessage"] == "成功":
                                ...
                                result_findUserInfoByUserIds = findUserInfoByUserIds(uuids, groupId, uuid,
                                                                                     cookies=cookies)
                                assert result_findUserInfoByUserIds.response.status_code == 200
                            else:
                                logger.info("没有学生")
            if num_going_check:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_findUserInfoByUserIds.py"])
