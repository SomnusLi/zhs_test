...
class Test_unpublishedChatVotesList():
    """查询待发布投票列表"""

    @allure.story("用例--查询待发布投票列表")
    @allure.description("该用例查询待发布投票列表")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_unpublishedChatVotesList(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            result_unpublishedChatVotesList = unpublishedChatVotesList(groupId, uuid, cookies=cookies)
            assert result_unpublishedChatVotesList.response.status_code == 200
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_unpublishedChatVotesList.py"])
