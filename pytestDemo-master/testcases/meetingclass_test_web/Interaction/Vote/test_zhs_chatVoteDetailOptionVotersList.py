...
class Test_chatVoteDetailOptionVotersList():
    """查询投票选项对应的投票人信息情况"""

    @allure.story("用例--查询投票选项对应的投票人信息情况")
    @allure.description("该用例查询投票选项对应的投票人信息情况")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_chatVoteDetailOptionVotersList(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
            ...
            if result_screenCourseClassInteractionListV2.response.json()["rt"]["interactionListGoing"] == []:
                ...
                for ListGoing in result_screenCourseClassInteractionListV2.response.json()["rt"][
                    ...
                    if ListGoing["type"] == 2:
                        ...
                        logger.info("chatVoteDetailOptionCount")
                        result_chatVoteDetailOptionCount = chatVoteDetailOptionCount(groupId, voteId, uuid,
                                                                                     cookies=cookies)
                        assert result_chatVoteDetailOptionCount.response.status_code == 200
                        optionId = \
                            random.sample(result_chatVoteDetailOptionCount.response.json()["rt"]["voteOptions"], 1)[0][
                                "optionId"]
                        logger.info("chatVoteDetailOptionVotersList")
                        result_chatVoteDetailOptionVotersList = chatVoteDetailOptionVotersList(optionId, voteId, uuid,
                                                                                               cookies=cookies)
                        assert result_chatVoteDetailOptionVotersList.response.status_code == 200
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatVoteDetailOptionVotersList.py"])
