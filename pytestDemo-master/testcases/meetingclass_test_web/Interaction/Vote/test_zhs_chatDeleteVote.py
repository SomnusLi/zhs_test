...
class Test_chatDeleteVote():
    """删除投票"""

    @allure.story("用例--删除投票")
    @allure.description("该用例删除投票")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_chatDeleteVote(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
            ...
            if result_screenCourseClassInteractionListV2.response.json()["rt"]["interactionListGoing"] == []:
                ...
                for ListGoing in result_screenCourseClassInteractionListV2.response.json()["rt"][
                    ...
                    if ListGoing["type"] == 2:
                        ...
                        logger.info("chatDeleteVote")
                        result_chatDeleteVote = chatDeleteVote(voteId, uuid, cookies=cookies)
                        assert result_chatDeleteVote.response.status_code == 200
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_chatDeleteVote.py"])
