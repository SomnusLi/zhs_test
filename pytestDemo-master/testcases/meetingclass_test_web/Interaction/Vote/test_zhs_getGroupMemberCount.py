...
class Test_getGroupMemberCount():
    """查询群组数量"""

    @allure.story("用例--查询群组数量")
    @allure.description("该用例查询群组数量")
    ...
    @pytest.mark.single
    def test_zhs_getGroupMemberCount(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_getGroupMemberCount.py"])
