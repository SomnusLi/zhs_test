...
class Test_findRunningPreemptive():
    """查询群下正在进行的抢答"""

    @allure.story("用例--查询群下正在进行的抢答")
    @allure.description("该用例查询群下正在进行的抢答")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_findRunningPreemptive(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            logger.info("findRunningPreemptive")
            result_findRunningPreemptive = findRunningPreemptive(groupId, uuid, cookies=cookies)
            assert result_findRunningPreemptive.response.status_code == 200
            t = result_findRunningPreemptive.response.json()["rt"]["resultMessage"]
            if result_findRunningPreemptive.response.json()["rt"]["resultMessage"] == None:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_findRunningPreemptive.py"])
