...
class Test_quryRushAnswerDetail():
    """查询抢答详细"""

    @allure.story("用例--查询抢答详细")
    @allure.description("该用例查询抢答详细")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_quryRushAnswerDetail(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            if result_screenCourseClassInteractionListV2.response.json()["rt"]["interactionListGoing"] == []:
                ...
                for ListGoing in result_screenCourseClassInteractionListV2.response.json()["rt"][
                    ...
                    if ListGoing["type"] == 4:
                        ...
                        logger.info("quryRushAnswerDetail")
                        result_quryRushAnswerDetail = quryRushAnswerDetail(groupId, rushAnswerId, uuid, cookies=cookies)
            ...
            if result_screenCourseClassInteractionListV2.response.json()["rt"]["InteractionDetailDtos"] == []:
                ...
                for DetailDtos in result_screenCourseClassInteractionListV2.response.json()["rt"][
                    ...
                    if DetailDtos["type"] == 4:
                        num_ended_rushAnswer = True
                        rushAnswerId = DetailDtos["interactionMap"]["rushAnswerId"]
                        logger.info("quryRushAnswerDetail")
                        result_quryRushAnswerDetail = quryRushAnswerDetail(groupId, rushAnswerId, uuid, cookies=cookies)
                        assert result_quryRushAnswerDetail.response.status_code == 200
            ...
            if num_ended_rushAnswer:
                logger.info("有已结束的抢答")
            else:
                logger.info("没有已结束的抢答")
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_quryRushAnswerDetail.py"])
