...
class Test_startRushAnswer():
    """创建抢答"""

    @allure.story("用例--创建抢答")
    @allure.description("该用例创建抢答")
    ...
    @allure.title(
        "测试数据：上游业务获取")
    @pytest.mark.single
    def test_zhs_startRushAnswer(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            logger.info("getGroupMemberCount")
            result_getGroupMemberCount = getGroupMemberCount(groupId, uuid,
                                                             cookies=cookies)
            assert result_getGroupMemberCount.response.status_code == 200
            sumCount = result_getGroupMemberCount.response.json()["rt"]["sumCount"]
            if sumCount != 0:
                count = randomRangeNum(1, sumCount)
                logger.info("startRushAnswer")
                result_startRushAnswer = startRushAnswer(groupId, count, uuid, cookies=cookies)
                assert result_startRushAnswer.response.status_code == 200
            else:
                logger.info("该课程下无学生")
        else:
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_startRushAnswer.py"])
