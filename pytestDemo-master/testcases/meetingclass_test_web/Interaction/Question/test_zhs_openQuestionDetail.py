...
class Test_openQuestionDetail():
    """查询答疑详情"""

    @allure.story("用例--查询答疑详情")
    @allure.description("该用例查询答疑详情")
    ...
    @pytest.mark.single
    def test_zhs_openQuestionDetail(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            if result_checkExistQuestion.response.json()["rt"]["result"] == 1:
                ...
                logger.info("openQuestionDetail")
                result_openQuestionDetail = openQuestionDetail(rushQuestionId, uuid, cookies=cookies)
                assert result_openQuestionDetail.response.status_code == 200
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_openQuestionDetail.py"])
