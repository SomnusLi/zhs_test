...
class Test_checkExistQuestion():
    """查询是否存在进行中的提问"""

    @allure.story("用例--查询是否存在进行中的提问")
    @allure.description("该用例是查询是否存在进行中的提问")
    ...
    @pytest.mark.single
    def test_zhs_checkExistQuestion(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            if result_checkExistQuestion.response.json()["rt"]["result"] == 1:
                logger.info(
                    "有正在进行中的提问，提问id为{}".format(result_checkExistQuestion.response.json()["rt"]["rushQuestionId"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_checkExistQuestion.py"])
