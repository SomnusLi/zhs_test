...
class Test_countJoinNum():
    """统计讨论参与人数"""

    @allure.story("用例--统计讨论参与人数")
    @allure.description("该用例统计讨论参与人数")
    ...
    @pytest.mark.single
    def test_zhs_countJoinNum(self, login_fixture_teacher):
        logger.info("*************** 开始执行用例 ***************")
        # login_fixture前置登录
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            if result_checkExistQuestion.response.json()["rt"]["result"] == 1:
                ...
                logger.info("countJoinNum")
                result_countJoinNum = countJoinNum(rushQuestionId, uuid, cookies=cookies)
                assert result_countJoinNum.response.status_code == 200
                if result_countJoinNum.response.json()["rt"]["resultStatus"] == 1:
                    logger.info("该提问参与人数为{}".format(result_countJoinNum.response.json()["rt"]["joinNum"]))
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_zhs_countJoinNum.py"])
