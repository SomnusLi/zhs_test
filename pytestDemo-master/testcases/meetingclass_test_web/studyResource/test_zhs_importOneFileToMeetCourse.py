...
class Test_importOneFileToMeetCourse():
    ...
    @allure.story("用例--选择学习资源里的文件")
    @allure.description("该用例是选择学习资源里的文件")
        ...
        if result_onlineservice_getStartingMeetCourseList.response.json()["rt"] != []:
            ...
            while Flag_Folder:
                ...
                if result_findFilesByFolderId.response.status_code == 200:
                    ...
                    if result_findFilesByFolderId.response.json()["rt"] != None:
                        Flag_Folder = False
            ...
            result_importOneFileToMeetCourse = importOneFileToMeetCourse(meetCourseId, dataId, uuid, cookies=cookies)
